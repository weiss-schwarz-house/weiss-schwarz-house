---
layout: default
title: 'HacKClaD -ハッククラッド'
players_min: 1
players_max: 4
time_min: 120
time_max: 120
tags:
  - 'ボードゲーム'
  - 'ソロ'
  - '協力ゲーム'
  - '対戦'
  - 'デッキビルディング'
thumbnail_url: 'https://baseec-img-mng.akamaized.net/images/item/origin/c32c544b4b5b7490733641d2fcfa5c72.png?imformat=generic&q=90&im=Resize,width=640,type=normal'
thumbnail_alt: 'HacKClaD -ハッククラッド サムネイル'
source_note: 'ボードゲームリスト.xlsm から作成した仮ページ'
---

## 概要メモ

- プレイ人数: 1〜4 人
- プレイ時間: 120 分
- 形式: ソロ/対戦/協力
- メカニクス: デッキビルディング
- 拡張: あり

## 拡張メモ

- HacKClaD PortableEdition -ハッククラッド
- HacKClaD EXTRA CLAD DECKS
- HacKClaD CROSS FATE ハッククラッド クロスフェイト
- HacKClaD.DeltA（ハッククラッド　デルタ）

## 備考

- HacKClaD 未来視マーカー
- HacKClaD未来視マーカー　レギオン
- HacKClaD.DeltAスリーブ（６種・各20枚）

参照元: {{ page.source_note }}
