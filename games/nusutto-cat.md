---
date: 2026-03-28
layout: default
title: Meow Heist（ぬすっとキャット）
toc: true
players_min: 3
players_max: 6
time_min: 15
time_max: 15
age: 6+
tags:
  - パーティーゲーム
  - カードゲーム
rulebook_url: "https://gamemarket.jp/game/186162"
thumbnail_url: "https://www.dropbox.com/scl/fi/xrur20vmqrbsrz2f1pynd/.png?rlkey=tffavsmhzm3m22m9ip2m41mmn&st=dmmoksgo&raw=1"
thumbnail_alt: "ぬすっとキャット サムネイル"
---

## ルール

- 商品ページ（下記）を参照してください。

- [ぬすっとキャット 商品ページ（Gamemarket）]({{ page.rulebook_url }})

## 概要メモ

- プレイ人数: 3〜6人
- プレイ時間: 約15分
- 対象年齢: 6歳〜
- デザイン: イブインク
- 価格: 2,500円（掲載情報に基づく）

短時間で遊べるカード系のパーティーゲーム。商品ページにルール説明や画像、コンポーネント写真が掲載されていますので、詳細はそちらをご確認ください。

参照元: [ぬすっとキャット 商品ページ（Gamemarket）]({{ page.rulebook_url }})
